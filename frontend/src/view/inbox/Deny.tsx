...
export default function Deny({ ticket, onBack, onSuccess }: DenyProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchWithToken(`http://localhost:5096/api/ticket/${ticket.ticketID}/reject`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        console.error("Gagal menolak tiket.");
      }
    } catch (error) {
        ...
        >
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-[#e8eff7] border border-[#cbd6e6] rounded-xl px-4 py-3 text-gray-800 outline-none text-sm resize-none focus:ring-2 focus:ring-red-400"
              required
            />
          </div>
          ...
          <div className="flex justify-center pt-2">
            <button 
              ...
              className="flex items-center gap-2 bg-[#ef4444] hover:bg-[#dc2626] text-white px-8 py-3 rounded-xl font-semibold shadow-md transition-colors text-sm disabled:opacity-50"
            >
              {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <>Tolak <XCircle size={18} /></>}
            </button>
          </div>
        </form>
      </div>
